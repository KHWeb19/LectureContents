public class MyCat {
    ...
    public void initMyCat (int age, float weight) {
        ...
        scheduleNum = new int[SCHEDULE_NUM];
    }
    private void feed () {
    ...
    private void warningMessage () {
        ...
        if (weight > 8) {
            System.out.printf("운동 안하면 고양이 비만 됩니다: %.1f", weight);
        } else if (weight < 6) {
            System.out.printf("너무 많이 빼도 위험합니다: %.1f", weight);
        }
    }
    ...
    public void simulation () {
        ...
        while (true) {
            ...
            for (int i = 0; i < SCHEDULE_NUM; i++) {
                scheduleNum[i] = getRandomPattern();

                switch (scheduleNum[i]) {
                    case FEED:
                        feed();
                        break;
                    case TRAINING:
